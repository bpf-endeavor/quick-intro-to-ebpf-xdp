...
int main(int argc, char *argv[])
{
	...
	int ret;
	struct third *skel = third__open_and_load();
	if (skel == NULL) {
		fprintf(stderr, "Failed to open and load the program\n");
		return EXIT_FAILURE;
	}

	// Load same values into the table
	struct key K = {};
	struct value V = {};
	strcpy(K.data, "hello world\n");
	strcpy(V.data, "from ebpf/xdp\n");
	bpf_map__update_elem(skel->maps.table, &K, sizeof(K), &V, sizeof(V), BPF_ANY);
